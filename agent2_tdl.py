...
from backgammon_value_net import (
    BackgammonValueNet, BOARD_LENGTH,
    CONV_INPUT_CHANNELS, AUX_INPUT_SIZE
)
...
def batch_encode_states(states, players):
    ...
    planes_batch = np.zeros(
        (batch_size, BOARD_LENGTH, CONV_INPUT_CHANNELS), dtype=np.float32
    )
...
def create_batch_value_function(model, params):  # for 2-ply search
...
def compute_value_gradients(params, model, planes, aux):
    ...
    batched_grad = jax.vmap(
        lambda pl, au: jax.grad(
            lambda p: single_value_fn(p, pl, au)
        )(params),
        in_axes=(0, 0)
    )
...
def true_online_td_lambda_update(
    params, traces, grads, values, next_values, rewards,
    alpha, lambda_param, gamma
):
    td_errors = rewards + gamma * next_values - values

    def update_trace_single(z, g):
        dot_prod = jnp.sum(z * g)
    ...
    new_traces = jax.tree.map(
        lambda z, g: jax.vmap(update_trace_single)(z, g),
        traces, grads
    )
    ...
    final_updates = jax.tree.map(
        lambda u: jnp.sum(u, axis=0),
        updates
    )
    new_params = optax.apply_updates(params, final_updates)
...
def train_agent2(
    ...
    print(
        f"Training Agent 2: batch={batch_size}, iters={num_iterations}, "
        f"lr={learning_rate}, λ={lambda_param}"
    )

    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_checkpoint_dir = (
        pathlib.Path(CHECKPOINT_DIR) / f"run_{run_id}"
    )
    ...
    for iteration in range(num_iterations):
        ...
        prev_planes, prev_aux = batch_encode_states(
            prev_states, prev_players
        )

        batch_value_fn = create_batch_value_function(model, params)
        moves = _vectorized_2_ply_search(
            states, players, dices, batch_value_fn
        )
        new_states = _vectorized_apply_move(
            states, players, moves
        )
        ...
        next_planes, next_aux = batch_encode_states(
            new_states, players
        )
        next_pred = model.apply(
            {'params': params}, next_planes, next_aux
        )
        next_values = (next_pred * 3.0).flatten()
        next_values = np.array(next_values, dtype=np.float32)
        next_values[game_over] = 0.0

        prev_values_pred = model.apply(
            {'params': params}, prev_planes, prev_aux
        )
        prev_values_pred = (prev_values_pred * 3.0).flatten()
        prev_values_pred = np.array(
            prev_values_pred, dtype=np.float32
        )

        grads = compute_value_gradients(
            params, model, prev_planes, prev_aux
        )
        ...
        if np.any(game_over):
            ...
            def reset_finished_traces(z):
                mask = (
                    1.0 - game_over_jax
                ).reshape((batch_size,) + (1,) * (z.ndim - 1))
                return z * mask

            traces = jax.tree.map(
                reset_finished_traces, traces
            )
        ...
        for i in range(batch_size):
            if game_over[i]:
                new_state, new_player, new_dice = (
                    _vectorized_new_game(1)
                )
        ...
        if (iteration + 1) % verbose_every == 0:
            if total_games > 0:
                white_win_rate = (
                    white_wins / total_games * 100
                )
                print(
                    f"[{iteration + 1}/{num_iterations}] "
                    f"Loss: {loss:.6f} | Games: {total_games} | "
                    f"White: {white_win_rate:.1f}% "
        ...
        if (
            ...
            checkpointer.save(
                run_checkpoint_dir / f"checkpoint_{iteration + 1}",
                params,
                force=True
            )
